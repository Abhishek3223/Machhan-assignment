...
const CartProvider = ({ children }) => {
    ...
    try {
      if (localStorage.getItem('authToken')) {
        ...
        const productDetailsPromises = cartData.Data.map(async (item) => {
          const response = await fetch(`https://fakestoreapi.com/products/${item.data_ID}`);
          if (!response.ok) {
            throw new Error(`Failed to fetch product details for item with data_ID: ${item.data_ID}`);
          }
  ...
  const addToCart = async (dataId, quantity) => {
    try {
      ...
      alert(error.message || 'Error while adding item to wishlist. Please try again later.');
    }
  };
